...
def calculate_support_resistance(data, window=12):
    ...
    low_idx = argrelextrema(low, np.less, order=17)[0]
    high_idx = argrelextrema(high, np.greater, order=17)[0]
    ...
    mth_low_idx = argrelextrema(mth_low, np.less, order=10)[0]
    mth_high_idx = argrelextrema(mth_high, np.greater, order=10)[0]
    ...
    sup_dates = data.index[data['Low'].isin(support)].tolist()
    res_dates = data.index[data['High'].isin(resistance)].tolist()
    print(support)
...
if show_chart:
    ...
    total_height = 650 + (num_subcharts * 30)  # Increase height for each subchart
    ...
    def update_chart_data():
        ...
        with chart_placeholder.container():
            chart_obj = StreamlitChart(height=total_height, toolbox=True)

            if apply_patterns:
                support, resistance, supp_dates, res_dates = calculate_support_resistance(replay_data)
                for sup_level, sup_date in zip(support, supp_dates):
                    chart_obj.horizontal_line(sup_level, color='green')
                    chart_obj.marker(time=sup_date, position='below', shape='arrow_up', color='green', text='S')
                for res_level, res_date in zip(resistance, res_dates):
                    chart_obj.horizontal_line(res_level, color='red')
                    chart_obj.marker(time=res_date, position='above', shape='arrow_down', color='red', text='R')
