...
def strategy_tester():
    ...
    with st.sidebar.expander("📂 Database Parameters"):
        table_name = st.text_input("Table Name", value="SBIN")
    ...
    if st.sidebar.button("🚀 Run Strategies"):
        with st.spinner("Running strategies..."):
            ...
            benchmark_cagr = (data['Close'].iloc[-1] / data['Close'].iloc[0]) ** (1 / ((data['Date'].iloc[-1] - data['Date'].iloc[0]).days / 365.25)) - 1
        ...
        st.line_chart(data.set_index('Date')[['DynamicPortfolioValue', 'FixedPortfolioValue', 'BenchmarkValue']])
