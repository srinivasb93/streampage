...
import class_utils.ConfigManager as ConfigManager
import class_utils.Utility as Utility
import class_utils.Fetcher as Fetcher
...
from class_utils.OtaUpdater import OTAUpdater
from class_utils.Changelog import VERSION
    ...
    try:
        ...
        if type(execute_inputs[0]) == str or int(execute_inputs[0]) < 15:
            ...
            try:
                ...
                url_dict_reversed = {key.replace('^', '').replace('.NS', ''): value for key, value in
                                     fetcher.getAllNiftyIndices().items()}
                url_dict_reversed = {v: k for k, v in url_dict_reversed.items()}
                df.index = df.index.map(
                    lambda x: "https://in.tradingview.com/chart?symbol=NSE%3A" + url_dict_reversed[x])
                url_dict_reversed = {v: k for k, v in url_dict_reversed.items()}
                df.index = df.index.map(
                    lambda x: f'<a href="{x}" target="_blank">{url_dict_reversed[x.split("%3A")[-1]]}</a>')
            except KeyError:
                pass
        else:
...
def on_start_button_click():
    ...
    if Utility.tools.isBacktesting(backtestDate=backtestDate):
        st.write(
            f'Running in :red[**Backtesting Mode**] for *T = {str(backtestDate)}* (Y-M-D) : [Backtesting data is subjected to availability as per the API limits]')
    ...
    while int(os.environ.get('SCREENIPY_SCREEN_COUNTER')) < 100:
        ...
        if os.environ.get('SCREENIPY_REQ_ERROR') and "TRUE" in os.environ.get('SCREENIPY_REQ_ERROR'):
            ...
            ac.info(
                'This issue is related with your Internet Service Provider (ISP) - Many **Jio** users faced this issue as the screeni-py data cache server appeared to be not reachable for them!\n\nPlease watch the YouTube video attached here to resolve this issue on your local system\n\nTry with another ISP/Network or go through this thread carefully to resolve this error: https://github.com/pranjal-joshi/Screeni-py/issues/164',
                icon='ℹ️')
...
def nifty_predict(col):
    with col.container():
        with st.spinner('🔮 Taking a Look into the Future, Please wait...'):
            import class_utils.Fetcher as Fetcher
            import class_utils.Screener as Screener
            configManager = ConfigManager.tools()
    ...
    col.warning(
        'The AI prediction should be executed After 3 PM or Around the Closing hours as the Prediction Accuracy is based on the Closing price!\n\nThis is Just a Statistical Prediction and There are Chances of **False** Predictions!',
        icon='⚠️')
    col.info(
        "What's New in **v3**?\n\nMachine Learning model (v3) now uses Nifty, Crude and Gold Historical prices to Predict the Gap!",
        icon='🆕')
...
def get_extra_inputs(tickerOption, executeOption, c_index=None, c_criteria=None, start_button=None):
    ...
    if not tickerOption.isnumeric():
        ...
        if select_pattern == 1 or select_pattern == 2:
            ...
            confluence_percentage = c2.number_input('MA Confluence %', min_value=0.1, max_value=5.0, value=1.0,
                                                    step=0.1, format="%1.1f") / 100.0
...
st.subheader('Stock Screener.. Find Breakouts, Just in Time!')


tab_screen, tab_similar, tab_nifty, tab_config, tab_psc, tab_about = st.tabs(
    ['Screen Stocks', 'Search Similar Stocks', 'Nifty-50 Gap Prediction', 'Configuration', 'Position Size Calculator',
     'About'])
...
with tab_screen:
    ...
    tickerOption = str(12 if '>' not in tickerOption else int(tickerOption[0]) if tickerOption[0].isnumeric() else str(
        tickerOption[0]))
    picked_date = c_datepick.date_input(label='Screen/Backtest For', max_value=datetime.date.today(),
...
with tab_config:
    ...
    period = ac.selectbox('Period', options=period_options, index=period_options.index(configManager.period),
                          placeholder='300d / 52wk')
    ...
    duration = cc.selectbox('Candle Duration', options=duration_options,
                            index=duration_options.index(configManager.duration), placeholder='15m / 1d / 1wk')
    if 'm' in duration or 'h' in duration:
        cc.write('For Intraday duartion, Max :red[value of period <= 60d]')

    ac, bc = st.columns([1, 1])
    minprice = ac.number_input('Minimum Price (Stocks below this will be ignored)', value=float(configManager.minLTP),
                               step=0.1)
    maxprice = bc.number_input('Maximum Price (Stocks above this will be ignored)', value=float(configManager.maxLTP),
                               step=0.1)

    ac, bc = st.columns([1, 1])
    volumeratio = ac.number_input('Volume multiplier for Breakout confirmation', value=float(configManager.volumeRatio),
                                  step=0.1)
    consolidationpercentage = bc.number_input('Range consolidation (%)',
                                              value=int(configManager.consolidationPercentage), step=1)
    ...
    cache = bc.checkbox('Enable caching of stock data after market hours', value=configManager.cacheEnabled,
                        disabled=True)
    stagetwo = cc.checkbox(
        'Screen only for [Stage-2](https://www.investopedia.com/articles/investing/070715/trading-stage-analysis.asp#:~:text=placed%20stops.-,Stage%202%3A%20Uptrends,-Image%20by%20Sabrina) stocks',
        value=configManager.stageTwo)
...
with tab_similar:
    ...
    candles = bc.number_input('Lookback Period (No. of Candles)', min_value=1, step=1,
                              value=int(configManager.daysToLookback))
    ...
    if similar_search_button:
        ...
        if result:
            with st.container():
                show_df_as_result_table()
                st.write(
                    'Click [**here**](https://medium.com/@joshi.pranjal5/spot-your-favourite-trading-setups-using-vector-databases-1651d747fbf0) to know How this Works? 🤔')

with tab_about:
    from class_utils.Changelog import VERSION, changelog
...
🎬 YouTube Playlist: Watch [**Here**](https://youtube.com/playlist?list=PLsGnKKT_974J3UVS8M6bxqePfWLeuMsBi&si=b6JNMf03IbA_SsXs) [![YouTube Channel Subscribers](https://img.shields.io/youtube/channel/subscribers/UCb_4n0rRHCL2dUbmRvS7psA)](https://www.youtube.com/@PranjalJoshi)
          """)
    bc.write(
        '<iframe width="445" height="295" src="https://www.youtube.com/embed/videoseries?si=aKXpyKKgwCcWIjhW&amp;list=PLsGnKKT_974J3UVS8M6bxqePfWLeuMsBi" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>',
        unsafe_allow_html=True)
...
with tab_psc:
    ...
    ac, bc = st.columns([1, 1])
    capital = ac.number_input(label='Capital Size', min_value=0, value=100000,
                              help='Total Amount used for Trading/Investing')
    if capital:
        in_words = num2words(capital, lang='en_IN').title()
        bc.write(f"<p style='margin-top:35px; font-weight: bold;'>Your Capital is Rs. {in_words}</p>",
                 unsafe_allow_html=True)

    risk = ac.number_input(label="% Risk on Capital for this trade", min_value=0.0, max_value=10.0, step=0.1, value=0.5,
                           help='How many percentage of your total capital you want to risk if your Stoploss hits? If you want a max loss of 1000 for an account value of 100,000 then your risk is 1%. It is not advised to take Risk more than 5% per trade! Think about your maximum loss before you trade!')
    if risk:
        risk_rs = capital * (risk / 100.0)
        in_words = num2words(risk_rs, lang='en_IN').title()
        bc.write(f"<p style='margin-top:40px; font-weight: bold;'>Your Risk for this trade is Rs. {in_words}</p>",
                 unsafe_allow_html=True)

    ac.divider()

    sl = ac.number_input(label="Stoploss in points", min_value=0.0, step=0.1,
                         help='Stoploss in Points or Rupees calculated by you by analyzing the chart.')
    if sl > 0:
    ...
    if calculate_qty_btn:
        if sl > 0:
            ...
            oc.metric(label='Quantity', value=qty, delta=f'Max Loss: {(-1 * qty * sl)}', delta_color='inverse',
                      help='Trade this Quantity to prevent excessive unplanned losses')
        elif price > 0 and percentage_sl > 0:
            qty = floor(risk_rs / actual_sl)
            oc.metric(label='Quantity', value=qty, delta=f'Max Loss: {(-1 * qty * actual_sl)}', delta_color='inverse',
                      help='Trade this Quantity to prevent excessive unplanned losses')
