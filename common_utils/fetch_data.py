...
def fetch_stock_or_index_data(symbol, start_date, end_date):
    query = f"SELECT Date, [Close] FROM dbo.{symbol} WHERE Date between '{start_date}' AND '{end_date}' ORDER BY Date"

    # Fetch data from SQL Server
    stock_data = rd.get_table_data(query=query)
    stock_data.set_index(keys='Date', inplace=True)
    stock_data['Pct_Chg'] = stock_data['Close'].pct_change()
    stock_data['Cum_Return'] = round(((1 + stock_data['Pct_Chg']).cumprod() - 1)*100, 1)
...
def fetch_benchmark_data(index_symbol, start_date, end_date):
    # Replace 'your_connection_string' with your actual SQL Server connection string
    query = f"SELECT Date, [Close] FROM dbo.{index_symbol} WHERE Date between '{start_date}' AND '{end_date}' ORDER BY Date"
...
def fetch_portfolio_data(symbols, start_date):
    portfolio_data = pd.concat([fetch_stock_or_index_data(symbol, start_date) for symbol in symbols], axis=1)
...
if __name__ == '__main__':
    print(fetch_stock_or_index_data('TATAMOTORS', start_date=dt.date(2024, 1, 1), end_date=dt.date.today()))