...
def fetch_data_from_db(table_name, start_date, end_date):
    query = f"SELECT * FROM \"{table_name}\" WHERE timestamp BETWEEN '{start_date}' AND '{end_date}' ORDER BY timestamp ASC"
...
def backtest_dynamic_strategy(data, fixed_investment, drawdown_threshold=-5):
    ...
    for year_month in unique_year_months:
        ...
        if investments_made == 0:
            ...
            # Record the trade
            portfolio['trades'].append({
                'timestamp': last_working_row['timestamp'],
...
def backtest_fixed_investment(data, fixed_investment, investment_day=15):
    ...
    for year_month in unique_year_months:
        ...
        # Record the trade
        portfolio['trades'].append({
            'timestamp': investment_row['timestamp'].iloc[0],
...
def calculate_buy_hold_returns(data):
    ...
    num_years = (data['timestamp'].iloc[-1] - data['timestamp'].iloc[0]).days / 365.25
...
def main():
    ...
    etf_data = fetch_data_from_db('ICICIB22', start_date, end_date)
