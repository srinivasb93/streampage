...
try:
    ...
    class StockChartApp(QMainWindow):
        ...
        def setup_chart(self, apply_indicators=False, apply_patterns=False):
            if self.data_source == 'SQL':
                postfix = ''
            ...
            if isinstance(self.stock_data, pd.DataFrame) and not self.stock_data.empty:
                ...
                if apply_indicators:
                    if self.indicators:
                        self.apply_indicator(direct_call=False)
                ...
                if apply_patterns and self.chart.lines():
                    ...
                    self.apply_patterns(direct_call=False)
                ...
                if self.replay_index < len(self.replay_data):
                    ...
                    for indicator in self.indicators:
                        if indicator['type'] in ['EMA', 'Lin_Reg']:
                            indicator_line = self.chart.create_line(f"{indicator['type']}_{indicator['period']}")
                            indicator_line.set(current_data[['time', f"{indicator['type']}_{indicator['period']}"]])
                        elif indicator['type'] == 'RSI' and self.rsi_chart:
                            rsi_line = self.rsi_chart.create_line(f"RSI_{indicator['period']}")
                            rsi_line.set(current_data[['time', f"RSI_{indicator['period']}"]])
                            ...
                            macd_data = current_data[['time', 'MACD', 'Signal', 'Histogram']]
        ...
        def calculate_indicators(self, data):
            for indicator in self.indicators:
                if indicator['type'] == 'EMA':
                    data[f'EMA_{indicator["period"]}'] = data['close'].ewm(span=indicator['period']).mean()
                elif indicator['type'] == 'Lin_Reg':
                    data[f'Lin_Reg_{indicator["period"]}'] = ta.linreg(data['close'], length=indicator['period'])
                elif indicator['type'] == 'RSI':
                    data[f'RSI_{indicator["period"]}'] = ta.rsi(data['close'], timeperiod=indicator['period'])
        ...
        def apply_indicator(self, dialog=None, direct_call=True):
        ...
        def apply_patterns(self, direct_call=True):
