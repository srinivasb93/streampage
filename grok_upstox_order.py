...
def place_order(api, instrument_token, transaction_type, quantity, price=0, order_type="MARKET",
    ...
    try:
        ...
        notify(f"Order Placed: {transaction_type}", f"Order ID: {response.data.order_id}")
        return response
...
with st.sidebar:
    ...
    st.session_state['sandbox_mode'] = st.checkbox("Enable Sandbox", value=False)
...
if page == "Get Token":
    if st.session_state['sandbox_mode']:
        st.subheader("Fetch Upstox Access Token of Sandbox")
        st.session_state["access_token"] = SANDBOX_ACCESS_TOKEN
        st.success(f"Successfully fetched Access Token of Sandbox")
    else:
...
# Combined Order & Risk Management Page
    ...
    with col1:
        ...
        with funds_cols[1]:
            ...
            subscribed_list = st.selectbox("Subscribed instruments", options=[""] + subscribed_list,
        ...
        if single_multi_order == 'Single Order':
            ...
            if instrument_token not in subscribed_list:
                if is_connected():
                    subscribe_to_instrument(instrument_token)
                    st.write(f"Auto-subscribed to {instrument_token}")
                else:
                    st.write("Waiting for WebSocket connection to auto-subscribe...")
                ...
                with st.spinner("Processing order..."):
                    if other_orders == "Auto-sell if Open > Previous Close":
                        ...
                        if schedule_datetime > datetime.now():
                            # Define the scheduled function with explicit arguments
                            def execute_scheduled_order(symbol_token, trans_type, qty, price_val, order_type_val,
                                ...
                                result = place_order(apis["order"], symbol_token, trans_type, qty, price_val,
                                                     order_type_val, trigger_val, amo, prod_type)
                                if result:
                                    st.success(f"Scheduled order executed: Order ID {result.data.order_id}")


                            # Pass current values to the thread
                            thread = threading.Thread(
                                target=execute_scheduled_order,
                                args=(instrument_token, transaction_type, quantity, price, order_type,
    ...
    if single_multi_order == 'Multiple Orders':
        ...
        for i in range(num_orders):
            ...
            with st.expander(f"Details for Order {i + 1}", expanded=True):
                ...
                orders.append(upstox_client.MultiOrderRequest(quantity=quantity,
                                                              product= product_type,
                                                              validity="DAY",
                                                              price=price,
        ...
        if st.button("Place Multiple Orders"):
            with st.spinner("Placing multiple orders..."):
                try:
                    response = apis["order"].place_multi_order(orders)
                    st.success("Multiple orders placed successfully!")
                    st.json(response.data)  # Display the response
                except ApiException as e:
...
st.markdown(f'<div class="metric-box">Last Updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</div>',
            unsafe_allow_html=True)
if st.session_state["sandbox_mode"]:
    st.markdown(f'<div class="metric-box">In Sandbox Mode</div>',
                unsafe_allow_html=True)