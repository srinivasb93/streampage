...
from common_utils import read_write_sql_data as rd  # Assuming your custom module
from python_scripts.stocks_data_load.daily_data_load import Dataload  # Assuming your custom module
import time
...
st.markdown("""
    <style>
    .main { padding: 10px; }
    .stButton>button { background-color: #4CAF50; color: white; border-radius: 5px; margin: 2px; }
...
def extract_stock_data(stock_name, data_source='SQL', period_sql='Daily', period_yf='1y', interval_yf='1d',
    ...
    if data_source == 'SQL':
        ...
        instrument_token = f"NSE_EQ|INE669E01016"  # Adjust as per Upstox instrument keys
        to_date = pd.Timestamp.now().strftime('%Y-%m-%d')
        response = apis["history"].get_historical_candle_data(instrument_token, period_upstox, to_date, api_version='v2')
...
def calculate_stock_summary(stock_df):
    ...
    if len(stock_df) > 50:
        stock_df['EMA_50'] = ta.ema(stock_df["Close"], length=50)  # Added for crossover
    if len(stock_df) > 200:
...
with st.sidebar:
    ...
    data_src = st.radio("Data Source", ["SQL", "Yahoo", "Upstox"], horizontal=True)
    ...
    if data_src == 'SQL':
        ...
        yf_period = st.selectbox("Period", ("1y", "5d", "1mo", "3mo", "6mo", "1d", "2y", "5y", "10y", "ytd", "max"))
        yf_interval = st.selectbox("Interval", (
...
def update_chart_data():
    ...
    with chart_placeholder.container():
        ...
        if apply_patterns:
            support_data, resistance_data = calculate_support_resistance(replay_data)
...
if show_summary or show_heatmap or show_analysis or show_data:
    ...
    if show_summary:
        with tabs[tab_index]:
            ...
            support_data, resistance_data = calculate_support_resistance(summary_data)
        ...
        with tabs[tab_index]:
            ...
            if st.button("Generate Heatmap"):
                monthly_df = extract_stock_data(stock_name, data_source=data_src,
                                                period_sql="Monthly" if data_src == 'SQL' else "10y", interval_yf="1mo")
    ...
    if show_analysis:
        with tabs[tab_index]:
            ...
            if not common_index.empty:
                stock_data = df["Close"].reindex(common_index).fillna(method='ffill')
                bench_data = bench_df["Close"].reindex(common_index).fillna(method='ffill')
            ...
            with analysis_cols[1]:
                ...
                support_data, resistance_data = calculate_support_resistance(recent_data)
